...
enum class FutureState(val order: Int) {
    INVALID(0),
    ...
    // Future is cancelled.
    CANCELLED(3),
    // Future result is consumed and no longer available.
    CONSUMED(4)
}
...
class Future<T> {
    ...
    fun consume(code: (T) -> Unit) {
        when (state) {
            ...
            FutureState.INVALID, FutureState.CONSUMED ->
 ...
 */
// TODO: make me value class!
class Transferrable<T>(val value: NativePtr)
 ...
 */
fun <T1, T2> schedule(worker: Worker, producer: () -> Transferrable<T1>,
                      @VolatileLambda job: (T1) -> Transferrable<T2>): Future<T2> =
    /**
     * This function is a magical operation, handled by lowering in the compiler, and replaced with call to
     *   scheduleImpl(worker.id, producer, job)
...
internal fun scheduleImpl(worker: Worker, producer: () -> Transferrable<Any?>,
                          job: CPointer<CFunction<*>>) : Future<Any?> =
        Future<Any?>(scheduleInternal(worker.id, producer, job))
...
/**
 * Returns an object suitable for cross-worker transfer in schedule()'s producer.
 * Creates copy if needed for SAFE mode, may throw an IllegalStateException if transfer failed.
 */
fun <T> T.transfer(mode: TransferMode = TransferMode.CHECKED): Transferrable<T> =
        Transferrable<T>(transferInternal(this, mode.value))
...
external internal fun scheduleInternal(
        id: WorkerId, producer: () -> Transferrable<Any?>, job: CPointer<CFunction<*>>) : FutureId

@SymbolName("Kotlin_Worker_transferInternal")
external internal fun transferInternal(value: Any?, mode: Int) : NativePtr
...
@ExportForCppRuntime
internal fun WorkerLaunchpad(function: () -> Transferrable<Any?>): NativePtr = function().value

// TODO: remove, once Transferrable represented as native pointer.
@ExportForCppRuntime
internal fun TransferrableValue(transferrable: Transferrable<*>) = transferrable.value